...
app.post('/offers', function(req, res) {
  if (!req.body.title) {
    return res.status(400).send({
      success: false,
      msg: 'No appropriate data send to store offer!'
    });
  }
  ...
  newOffer.save(function(err) {
    ...
    res
      .status(201)
      .send({ success: true, msg: 'Successfully created new offer.' });
  });
});
...
app.post('/offercategory', function(req, res) {
  if (!req.body.name) {
    return res.status(400).send({
      success: false,
      msg: 'No appropriate data send to store offer category!'
    });
  }
  ...
  newOfferCategory.save(function(err) {
    if (err) {
      ...
      return res.json({
        success: false,
        msg: 'Error while creating offer category',
        error: err
      });
    }
    res
      .status(201)
      .send({ success: true, msg: 'Successfully created new offer category.' });
  });
});

// GET
// Get info / health state
app.get('/', (req, res) => {
  res.json({
    data: {
      type: 'Message',
...
app.get('/offers', function(req, res) {
  Offer.find({}, function(err, offers) {
    if (err) {
      return res.json({
        success: false,
        msg: 'Error while retrieving offers',
        error: err
      });
    }
...
app.post('/search/offers', function(req, res) {
  ...
  Offer.search(search, coordinates, distance, function(err, offers) {
    if (err) {
      return res.json({
        success: false,
        msg: 'No entries found. An error occurred.',
        error: err
      });
    }
...
app.post('/offersbylocation', function(req, res) {
  ...
  Offer.findByLocation(coordinates, distance, function(err, offers) {
    if (err) {
      return res.json({
        success: false,
        msg: 'No entry found near: ' + coordinates.toString(),
        error: err
      });
    }
